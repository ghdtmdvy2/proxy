...
public class FieldLogTrace implements LogTrace {
    ...
    private TraceId traceIdHolder; // traceId 를 동기화, 동시성 이슈 발생.

    @Override
    public TraceStatus begin(String message) {
        syncTraceId(); // TraceId 생성.
        TraceId traceId = traceIdHolder; // syncTraceId로 생성된 TraceId 로 저장.
    ...
    private void syncTraceId(){
        if (traceIdHolder == null) { // TraceId 가 없을 때는 새로 생성.
    ...
    private void releaseTraceId(){ // traceId 의 level 을 낮춰주는 것.
        if (traceIdHolder.isFirstLevel()) { // 첫번째 level 인 경우 TraceId 파괴
            traceIdHolder = null;
        } else {
            traceIdHolder = traceIdHolder.createPrevId(); // level 을 낮춰주기.
        }
    }
